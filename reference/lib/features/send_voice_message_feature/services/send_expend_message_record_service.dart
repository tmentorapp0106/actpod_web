...
class SendExpendMessageRecordService {
  final RecorderController _recorderController;
  final PlayerController _playerController;
  StreamSubscription<Duration>? recordTimerSub;
  StreamSubscription<int>? playerTimerSub;
  StreamSubscription<void>? playerCompleteSub;

  SendExpendMessageRecordService(this._recorderController, this._playerController);
  ...
  Future<Duration> playAudio(String audioPath, Function onCompletion, Function onCursorChange) async {
    ...
    if(playerCompleteSub != null) {
      playerCompleteSub!.cancel();
      playerCompleteSub = null;
    }
    playerCompleteSub = _playerController.onCompletion.listen((_) {
      _playerController.stopPlayer();
      onCompletion();
    });

    if(playerTimerSub != null) {
      playerTimerSub!.cancel();
      playerTimerSub = null;
    }
    playerTimerSub = _playerController.onCurrentDurationChanged.listen((durationMillisecond) {
