...
import 'package:quick_share_app/features/play_record_feature/components/delete_comment_dialog.dart';
                    ...
                    children: [
                      ...
                      Expanded(
                        child:AnimatedSwitcher(
                          ...
                          transitionBuilder: (child, animation) {
                            final isReply = child.key == const ValueKey('reply');

                            // 進場：依照目標頁決定從哪邊進來
                            final inTween = Tween<Offset>(
                              begin: isReply ? const Offset(1, 0) : const Offset(-1, 0),
                              end: Offset.zero,
                            );

                            // ✅ 對調：讓 reverse 時從 0 -> endOffset
                          ...
                          child: ref.watch(isReplyModeProvider)
                              ? _buildReplyList(scrollController, ref, key: const ValueKey('reply'))
                              : _buildCommentList(scrollController, ref, key: const ValueKey('comment')),
                        )
                      ),
                      ...
                      Padding(
                        ...
                        child: Column(
                          children: [
                            Visibility(
                              visible: ref.watch(inputFocusProvider) && !ref.watch(isReplyModeProvider),
                              child: Column(
                                children: [
                                  Row(
                                    children: [
            ...
            children: [
              Row(
                children: [
                  ...
                  Text(
                    TimeUtils.timeAgo(comment.commentTime)
                  )
                ],
              ),
              SizedBox(height: 6),
                    ...
                    children: [
                      Row(
                        children: [
                          ...
                          SizedBox(width: 8.w,),
                          Text(
                            TimeUtils.timeAgo(reply.replyTime)
                          )
                        ],
                      ),
                      // No inner Row needed
