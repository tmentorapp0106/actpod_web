...
class UpdatePodcastStoreScreen extends StatefulWidget {
  const UpdatePodcastStoreScreen({super.key});

  @override
  State<UpdatePodcastStoreScreen> createState() => _UpdatePodcastStoreScreenState();
}

class _UpdatePodcastStoreScreenState extends State<UpdatePodcastStoreScreen> {
  ...
  Future<void> _loadPodcastStore() async {
    ...
    try {
      /// 這裡換成你自己的 API
      /// 例如:
      /// final res = await channelApiManager.getPodcastStore();
      final res = await channelApiManager.getPodcastStore(UserService.getUserInfo()!.userId);
      ...
      final data = res.podcastStoreDto;
      ...
      _links.clear();
      final originalLinks = data?.links ?? [];
      if (originalLinks.isEmpty) {
        _links.add(LinkFormItem());
      } else {
        for (final item in originalLinks) {
          final formItem = LinkFormItem();
          formItem.urlController.text = item.url ?? "";
  ...
  Future<void> _pickStoreImage() async {
    File? selectFile =
    await ImageUtils.pickMedia(true, (file) => ImageUtils.cropImageFunc(file, 720, 720));
  ...
  Future<void> _pickLinkImage(int index) async {
    File? selectFile =
    await ImageUtils.pickMedia(true, (file) => ImageUtils.cropImageFunc(file, 50, 400));
    if (selectFile == null) return;
  ...
  Future<void> _submit() async {
    ...
    for(LinkFormItem link in _links) {
      if(link.urlController.text.isEmpty) {
    ...
    try {
      String finalStoreImageUrl = _storeImageUrl ?? "";

      if (_storeImageFile != null) {
        final uploadImageRes =
        await uploadApiManager.uploadPodcastStoreImage(File(_storeImageFile!.path));
        if (uploadImageRes.code != "0000" || uploadImageRes.data == null) {
        ...
        finalStoreImageUrl = uploadImageRes.data!.publicUrl;
      }

      for (final link in _links) {
        if (link.pickedImage != null) {
          final uploadImageRes =
          await uploadApiManager.uploadPodcastStoreImage(File(link.pickedImage!.path));
          if (uploadImageRes.code != "0000" || uploadImageRes.data == null) {
      ...
      /// 這裡換成你自己的更新 API
      /// 例如:
      /// final updateRes = await channelApiManager.updatePodcastStore(...)
      final UpdatePodcastStoreRes updateRes = await channelApiManager.updatePodcastStore(
      ...
      if (updateRes.code != "0000") {
        ToastService.showNoticeToast("更新 Podcast Store 失敗");
        setState(() {
          _isLoading = false;
        });
        return;
      ...
      return Scaffold(
        ...
        appBar: PreferredSize(
          ...
          child: AppBar(
            ...
            title: const Text("編輯 Podcast Store"),
            backgroundColor: Colors.white,
            ...
            IconButton(
              onPressed: () {
                ...
                SharePlus.instance.share(
                  ShareParams(
                    text: 'https://web.actpodapp.com/podcast_store/${UserService.getUserInfo()!.userId}?openExternalBrowser=1',
              ...
              _SectionCard(
                child: Column(
                  ...
                  children: [
                    ...
                    _buildImagePreview(
                      localImagePath: _storeImageFile?.path,
                      remoteImageUrl: _storeImageUrl,
                      emptyText: '尚未選擇商店圖片',
                    ),
                    const SizedBox(height: 12),
                    Row(
                      ...
                      return Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            _buildImagePreview(
                              localImagePath: item.pickedImage?.path,
                            ...
                            _buildTextField(
                              ...
                              validator: (value) {
                                ...
                                final hasImage = item.pickedImage != null ||
                                    (item.imageUrl != null && item.imageUrl!.trim().isNotEmpty);

                                if (hasImage && urlValue.isEmpty) {
                                  return '有圖片時請輸入對應連結';
                                }
                                return null;
                              },
