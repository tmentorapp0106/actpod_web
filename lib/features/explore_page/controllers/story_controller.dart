...
class StoryController {
  ...
  Future<void> init() async {
    GetRecommendationRes recommendationResponse =
        await recommendationApiManager.getRecommendationList(1, 20);
    if (recommendationResponse.code != "0000") {
      ToastService.showNoticeToast("找不到內容");
      return;
    }

    List<GetRecommendationResItem>? recommendationList =
        recommendationResponse.recordList;
    if (recommendationList != null) {
      ref.watch(storiesProvider.notifier).state =
          StoryInfoDto.fromRecommendationList(recommendationList);
    }

    if (AuthService.isLoggedIn()) {
      await getPurchasedStories();
    } else {
  ...
  Future<void> getPurchasedStories() async {
    FindUserPurchaseRecordsRes purchasedResponse =
        await storyApiManager.findUserPurchaseRecords();
    if (purchasedResponse.code != "0000") {
      return;
    }
    ref.watch(purchasedStoriesProvider.notifier).state =
        purchasedResponse.records;
  }
}
