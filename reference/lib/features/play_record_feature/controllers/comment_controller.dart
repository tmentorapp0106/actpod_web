...
import 'package:quick_share_app/apiManagers/comment_api_dto/create_reply_res.dart';
import 'package:quick_share_app/apiManagers/comment_api_dto/delete_comment_res.dart';
import 'package:quick_share_app/apiManagers/comment_api_dto/get_reply_by_comment_res.dart';
...
import 'package:quick_share_app/features/play_record_feature/components/instant_comment.dart';
...
import '../../../apiManagers/comment_api_dto/get_comment_list_res.dart';
import '../../../apiManagers/comment_api_dto/inseret_comment_res.dart';
import '../../../apiManagers/comment_system_api_manager.dart';
...
class CommentController {
  ...
  CommentController(this.ref, this. storyInfo);
  ...
  Future<void> createInstantComment(String storyId, String content, int sendSecond, int podcoins) async {
    ...
    instantCommentSendList.add(newInstantCommentInfo);
    instantCommentWaitingQueue.add(newInstantCommentInfo);
  ...
  Future<void> fireInstantComment(BuildContext context) async {
    if(isProcessingInstantComment || !ref.watch(instantCommentVisibilityProvider) || actPodAudioHandler?.mediaItem.value?.id != storyInfo.storyId || ref.watch(isPlayingInteractiveContentProvider)) {
      return;
    }
    ...
    if(ref.watch(mainPlayerStatusProvider) != MainPlayerState.playing) {
      while(instantCommentWaitingQueue.isNotEmpty) {
        ...
        tail = position.to;
        comments[(position.from / 10).toInt()] = InstantComment(
    ...
    int playingSecond = ref.watch(mainPlayerPositionProvider).inSeconds;
